// Execute this code when the DOM has finished loading.
$(document).ready(function() {
    // VARIABLES
    // ===================================================================
  
    // These object will hold our wrestlers.
    var wrestlers = {
      "Odwun The Viking": {
        name: "Odwun The Viking",
        health: 120,
        attack: 8,
        imageUrl: "assets/images/Odwun.png",
        enemyAttackBack: 15
      },
      "Derrick Jannetty": {
        name: "Derrick Jannetty",
        health: 100,
        attack: 14,
        imageUrl: "assets/images/DerrickJannetty.png",
        enemyAttackBack: 5
      },
      "Paradise": {
        name: "Paradise",
        health: 150,
        attack: 8,
        imageUrl: "assets/images/Paradise.png",
        enemyAttackBack: 20
      },
      "Guerrero Azteca": {
        name: "Guerrero Azteca",
        health: 180,
        attack: 7,
        imageUrl: "assets/images/guerrero.png",
        enemyAttackBack: 25
      }
    };
  
    // populates when the user selects a wrestler.
    var attacker;
    // Populates the remaining the wrestlers not selected.
    var roster = [];
    // populates the users chosen opponent.
    var defender;
    ...
    // this function will load all the wrestlers into the character section to be selected
    var initializeGame = function() {
      // Loop through the wrestlers object and call the renderCharacter function on each character to render their card.
      for (var key in wrestlers) {
        renderCharacter(wrestlers[key], "#wrestlers-section");
      }
    };
    ...
    // function will also place the character based on the areaRender chosen (e.g. #selected-wrestler or #defender)
    ...
    $("#wrestlers-section").on("click", ".character", function() {
      ...
      if (!attacker) {
        ...
        attacker = wrestlers[name];
        // We then loop through the remaining wrestlers and push them to the roster array.
        for (var key in wrestlers) {
          if (key !== name) {
            roster.push(wrestlers[key]);
          }
        }
  
        // Hide the character select div.
        $("#wrestlers-section").hide();
  
        // Then render our selected character and our roster.
        updateCharacter(attacker, "#selected-wrestler");
        renderEnemies(roster);
      }
    });
